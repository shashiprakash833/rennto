...
def update_request_status(request):
    ...
    try:
        ...
        if request.jwt_payload.get('role') == 'owner':
            ...
            is_hostel_change = request.data.get("is_hostel_change", False)
            ...
            if is_hostel_change:
                ...
                if status_val == "accepted":
                    ...
                    result = HostelChangeService.reject_change_request(request_id)
                else:
                    return Response({"error": "Invalid status for hostel change request"}, status=status.HTTP_400_BAD_REQUEST)
                    
                if result.get("success"):
                    return Response({"message": result.get("message")}, status=status.HTTP_200_OK)
...
def create_hostel_change_request(request):
    """Create a hostel change request when tenant wants to move to another hostel"""
    try:
        role = request.jwt_payload.get('role')
        if role != 'tenant':
            return Response(
                {"error": "Only tenants can create change requests"},
        ...
        message_to_owner = data.get('message_to_owner', '')
        tenant_name = data.get('tenant_name', '')
        tenant_email = data.get('tenant_email', '')
        requested_room_preference = data.get('requested_room_preference') or data.get('room_preference', '')
        additional_details = data.get('additional_details', '')
        
        result = HostelChangeService.create_change_request({
...
def check_hostel_booking_status(request, tenant_phone, target_hostel_id):
    """Check if tenant can book a hostel or if already staying"""
    try:
        result = HostelChangeService.check_can_book_hostel(
            tenant_phone,
            target_hostel_id
        )
        return Response(result, status=status.HTTP_200_OK)
...
def get_tenant_hostel_change_requests(request, tenant_phone):
    ...
    try:
        ...
        if role != 'tenant':
            return Response(
                {"error": "Only tenants can view their requests"},
                status=status.HTTP_403_FORBIDDEN
            )
