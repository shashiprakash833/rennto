...
class RequestService:
    ...
    def owner_requests(phone):
        ...
        hostel_changes = HostelChangeRequest.objects.filter(target_owner=owner).order_by('-created_at')
        for r in hostel_changes:
            data.append({
                "id": f"hc_{r.id}",
                "db_id": r.id,
                "name": r.tenant.name,
                "phone": r.tenant.phone,
                "status": r.status,
                "propertyName": r.target_hostel.hostelName,
                "propertyType": "hostel",
                "sharing": r.requested_room_preference,
                "checkIn": r.expected_joining_date.isoformat() if r.expected_joining_date else None,
                "created_at": r.created_at,
                "is_existing_tenant": True,
                "is_hostel_change": True,
                "current_hostel_name": r.current_hostel.hostelName,
                "days_remaining": r.days_remaining_in_current_hostel,
                "message": r.message_to_owner,
            })
