...
const OwnerNotificationScreen = ({ route }) => {
  ...
  const handleAction = async (action, id, isExisting = false, isHostelChange = false) => {
    try {
      const res = await fetchWithAuth(`${BASE_URL}/api/update_request_status/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: id, status: action, is_existing_tenant: !!isExisting, is_hostel_change: !!isHostelChange }),
      });
  ...
  const renderCard = (item) => {
    ...
    return (
      <View key={`${item.type}-${item.id}`} style={styles.card}>
        ...
        {item.is_hostel_change && (
          <View style={[styles.detailsGrid, { marginTop: 10, flexDirection: 'column', gap: 10 }]}>
            <Text style={{ fontSize: 12, fontWeight: "bold", color: COLORS.PRIMARY, marginBottom: 5 }}>Hostel Change Details</Text>
            
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Current Hostel</Text>
                <Text style={styles.detailValue}>{item.current_hostel_name || "Unknown"}</Text>
              </View>
              <View style={styles.detailItem}>
            ...
            {item.message ? (
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Message from Tenant</Text>
                <Text style={[styles.detailValue, { fontWeight: "normal", fontStyle: "italic", marginTop: 4 }]}>"{item.message}"</Text>
              </View>
            ) : null}
          </View>
        )}
        ...
        {status === "pending" ? (
            ...
            {item.type === "existing_tenant" ? (
              <>
                <TouchableOpacity
                  ...
                  onPress={() => handleAction("accepted", item.db_id || item.id, item.is_existing_tenant, item.is_hostel_change)}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleAction("rejected", item.db_id || item.id, item.is_existing_tenant, item.is_hostel_change)}
                >
          ...
          <View style={styles.actionButtons}>
            ...
            <TouchableOpacity style={[styles.btn, styles.declineBtn]} onPress={() => handleAction("rejected", item.db_id || item.id, item.is_existing_tenant, item.is_hostel_change)}>
              <Text style={styles.btnTextSecondary}>{t("decline") || "Decline"}</Text>
            </TouchableOpacity>
          </View>
        ) : null}
