...
export function PropertyDetailsScreen(props) {
  ...
  if (normalizedStatus === "pending") {
    ...
    buttonText = t("request_accepted") || "Request Accepted";
    buttonAction = "none";
    buttonDisabled = true;
    buttonColor = "#2ecc71";
  }
  ...
  // Hostel Change Request States
  const [bookNowModalVisible, setBookNowModalVisible] = useState(false);
  const [changeRequestFormVisible, setChangeRequestFormVisible] = useState(false);
  const [currentHostelForChange, setCurrentHostelForChange] = useState(null);
  const [targetHostelForChange, setTargetHostelForChange] = useState(null);
  const [availableHostels, setAvailableHostels] = useState([]);
  const { createChangeRequest, loading: changeRequestLoading, getAvailableHostels } = useHostelChangeRequest();

  // Fetch available hostels when form opens
  useEffect(() => {
    if (changeRequestFormVisible && availableHostels.length === 0) {
      getAvailableHostels()
        .then(hostels => setAvailableHostels(hostels))
  ...
  const confirmBooking = async () => {
    if (checkReadOnly()) return;
    try {
      if (isJoined && (!joinedProperty || jyoinedProperty.property_name?.trim() !== property.name?.trim())) {
    ...
    try {
      ...
      if (buttonAction === "book") {
        if (
          ...
          // Open Book Now modal for hostel change request
          setCurrentHostelForChange(joinedProperty);
          setTargetHostelForChange(property);
  ...
  const handleBookNowPress = async () => {
    ...
    setChangeRequestFormVisible(true);
  };
  ...
  const handleChangeRequestSubmit = async (formDataOrDate, message) => {
    try {
      ...
      if (typeof formDataOrDate === 'object') {
        ...
        expectedJoiningDate = formDataOrDate.expectedJoiningDate;
      ...
      if (!targetHostelId) {
        Alert.alert("Error", `Target Hostel ID is missing! Property name: ${property?.name}`);
        console.log("DEBUG: formDataOrDate:", formDataOrDate);
        console.log("DEBUG: targetHostelForChange:", targetHostelForChange);
      ...
      const tenantEmail = await AsyncStorage.getItem("tenantEmail") || "";
      if (!tenantPhone) {
        Alert.alert("Error", "Unable to get tenant phone number");
        return;
      }
      ...
      if (result?.success || result?.id) {
        setChangeRequestFormVisible(false);
        Alert.alert(
          "Success",
          "Your hostel change request has been sent to the owner. You will be notified once they respond."
        );
        // Reset form
        setCurrentHostelForChange(null);
        setTargetHostelForChange(null);
      } else {
        Alert.alert("Error", result?.error || result?.message || "Failed to send request");
      }
    } catch (error) {
    ...
    <View style={[styles.container, { backgroundColor: "#fff" }]}>
      ...
      <View style={styles.footer}>
        ...
        {normalizedStatus === "pending" ? (
          ...
          <View style={{ flex: 1, flexDirection: "column", paddingLeft: 12, paddingRight: 12, justifyContent: "center", gap: 10 }}>
            <TouchableOpacity
      ...
      <BookNowModal
        ...
        currentHostel={currentHostelForChange}
        targetHostel={targetHostelForChange}
        onBookNowPress={handleBookNowPress}
      />
      
      <ChangeHostelRequestForm
        visible={changeRequestFormVisible}
        onClose={() => setChangeRequestFormVisible(false)}
        currentHostel={currentHostelForChange || joinedProperty}
        targetHostel={targetHostelForChange || property}
