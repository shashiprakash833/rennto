...
export default function HostelScreen() {
  ...
  const handleApplyFilters = (filters) => {
    ...
    setSelectedHostelType(filters.hostelType || "");
...
const filteredHostels = useMemo(() => {
  const filtered = properties.filter((h) => {
    ...
    const matchesHostelType =
      selectedHostelType === "" ||
      (h.hostelType || "").toLowerCase() === selectedHostelType.toLowerCase();
  ...
  selectedHostelType, selectedFacilities, nearBy, userCoords, maxRent, sortBy, requests
]);
